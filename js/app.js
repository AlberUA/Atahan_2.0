...
***************************************************************************** */
        ...
        const txtMessageContactPage = document.querySelector(".form-contact__input_message");
        if (txtMessageContactPage) {
            const txtMessageContactPageLimit = txtMessageContactPage.getAttribute("maxlength");
            const txtMessageContactPageCounter = document.querySelector(".form-contact__message-counter span");
            txtMessageContactPageCounter.innerHTML = txtMessageContactPageLimit;
            txtMessageContactPage.addEventListener("keyup", txtMessageContactPageSetCounter);
            txtMessageContactPage.addEventListener("keydown", (function(event) {
                if (event.repeat) txtMessageContactPageSetCounter();
            }));
            function txtMessageContactPageSetCounter() {
                const txtCounterResult = txtMessageContactPageLimit - txtMessageContactPage.value.length;
                txtMessageContactPageCounter.innerHTML = txtCounterResult;
                if (txtMessageContactPageCounter.innerHTML < 300) txtMessageContactPageCounter.classList.add("color"); else txtMessageContactPageCounter.classList.remove("color");
            }
        }
        const txtMessagePost = document.querySelector(".form-post__input_message");
        if (txtMessagePost) {
            const txtMessagePostLimit = txtMessagePost.getAttribute("maxlength");
            const txtMessagePostCounter = document.querySelector(".form-post__count span");
            txtMessagePostCounter.innerHTML = txtMessagePostLimit;
            txtMessagePost.addEventListener("keyup", txtMessagePostSetCounter);
            txtMessagePost.addEventListener("keydown", (function(event) {
                if (event.repeat) txtMessagePostSetCounter();
            }));
            function txtMessagePostSetCounter() {
                const txtCounterResult = txtMessagePostLimit - txtMessagePost.value.length;
                txtMessagePostCounter.innerHTML = txtCounterResult;
                if (txtMessagePostCounter.innerHTML < 300) txtMessagePostCounter.classList.add("color"); else txtMessagePostCounter.classList.remove("color");
            }
        }
        const formImage = document.getElementById("imagePost");
        const imagesList = document.querySelector(".file-post__images-preload-list");
        if (formImage) formImage.addEventListener("change", (function previewImages(e) {
            ...
            for (e = 0; e < files.length; e++) {
                ...
                imgReader.addEventListener("load", (function(event) {
                    ...
                    img.classList.add("filePostPreload");
                    let iconClose = document.createElement("div");
                    iconClose.classList.add("fileClose");
                    li.appendChild(img);
                    li.appendChild(iconClose);
                    imagesList.appendChild(li);
                }));
                imgReader.readAsDataURL(files[e]);
                setTimeout((() => {
                    const removeimgPreload = document.querySelectorAll(".fileClose");
                    removeimgPreload.forEach((el => {
                        el.addEventListener("click", (function() {
                            el.closest("li").classList.add("remove");
                        }));
                    }));
                }), 500);
                const formImageContainerDiv = document.getElementById("images-post");
                formImageContainerDiv.classList.add("active");
            }
        }));
        const createPostButton = document.querySelector(".hero-wall__button");
        if (createPostButton) createPostButton.addEventListener("click", (function(e) {
            const postForm = document.querySelector(".page-wall__post-form");
